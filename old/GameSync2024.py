...
def GoveeLocalControl(
    ...
    try:
        if Command == "Color":
            ...
            byteArray = [187, 0, 32, 176, gradientFlag, 20]

            # Interpolate between color1 and color2
            for t in np.linspace(0, 1, 20):
                interpolated_color = (1 - t) * color1 + t * color2
                byteArray.extend(interpolated_color.astype(int))
...
def main(DeviceIP):
    ...
    try:
        ...
        while 1:
            ...
            gdi32.BitBlt(
                mem_dc, 0, 0, 1, 1, screen_dc, middle_x - chunk, middle_y, SRCCOPY
            )
...
try:
    ...
    GoveeLocalControl(Command="BrightLevel", UDP_IP=DeviceIP, brightness=30)
    time.sleep(1)
